...
int main(int argc, char **argv){
   ...
   error(strcmp(argv[2],"-p")!=0,1,"main ",
         "missing -p \n usage: ./form_factors blind/see/read_plateaux -p inpath");
   ...
   /*
   FILE *outfile        =NULL; mysprintf(namefile,"%s/out/out_E0.txt",argv[3]);        outfile=fopen(namefile,"w+");       error(outfile==NULL,1,"main ", "Unable to open %s file",namefile);
   FILE *m_pcac         =NULL; mysprintf(namefile,"%s/out/m_pcac.txt",argv[3]);        m_pcac=fopen(namefile,"w+");        error(m_pcac==NULL,1,"main ", "Unable to open %s file",namefile);
   FILE *outfile_RM     =NULL; mysprintf(namefile,"%s/out/Ratio_masses.txt",argv[3]);  outfile_RM=fopen(namefile,"w+");    error(outfile_RM==NULL,1,"main ",  "Unable to open %s file",namefile);
   FILE *outfile_GEVP   =NULL; mysprintf(namefile,"%s/out/out_E0_GEVP.txt",argv[3]);       outfile_GEVP=fopen(namefile,"w+");  error(outfile_GEVP==NULL,1,"main ",  "Unable to open %s file",namefile);
   FILE *outfile_RM_GEVP=NULL; mysprintf(namefile,"%s/out/Ratio_masses_GEVP.txt",argv[3]);  outfile_RM_GEVP=fopen(namefile,"w+");    error(outfile_RM_GEVP==NULL,1,"main ",  "Unable to open %s file",namefile);
   FILE *outfile_f      =NULL; mysprintf(namefile,"%s/out/f_PS.txt",argv[3]);          outfile_f=fopen(namefile,"w+");     error(outfile_f==NULL,1,"main ",  "Unable to open %s file",namefile);
   FILE *outfile_f_ls_ss      =NULL; mysprintf(namefile,"%s/out/f_PS_ls_ss.txt",argv[3]);          outfile_f_ls_ss=fopen(namefile,"w+");     error(outfile_f_ls_ss==NULL,1,"main ",  "Unable to open %s file",namefile);
   FILE *outfile_f_GEVP =NULL; mysprintf(namefile,"%s/out/f_PS_GEVP.txt",argv[3]);     outfile_f_GEVP=fopen(namefile,"w+");error(outfile_f==NULL,1,"main ", "Unable to open %s file",namefile);
   FILE *outfile_Rf     =NULL; mysprintf(namefile,"%s/out/Ratio_f_PS.txt",argv[3]);    outfile_Rf=fopen(namefile,"w+");    error(outfile_Rf==NULL,1,"main ",  "Unable to open %s file",namefile);
   */
   mysprintf(namefile,NAMESIZE," mkdir %s/Petros/",argv[3]);
   system(namefile);
   
   mysprintf(namefile,NAMESIZE,"%s/Petros/PP.txt",argv[3]);
   FILE *outfile_PP=open_file(namefile,"w+");
   FILE *outfile_PPi;
...
////////////////////////////////////////////////////////////////////
 ...
 //  fclose(f);
  ...
  // print_file_head(m_pcac); 
   ...
   double ***mass_jack_fit,***f_jack_fit,***mass_jack_fit_GEVP;   
...
//   for (ii=0;ii<si_2pt;ii++){
   ...
   for(imom1=0;imom1<file_head.nmoms;imom1++){
       ...
       mysprintf(namefile,NAMESIZE,"%s/Petros/PP_ik1_%.5f_ik2_%.5f.txt",argv[3],file_head.k[file_head.nk+ik1],file_head.k[file_head.nk+ik2]);
