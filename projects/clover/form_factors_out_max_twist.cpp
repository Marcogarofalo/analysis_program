...
void read_twopt(FILE *stream,int size, int iconf , double **to_write,int si, int ii, int imom2, int imom1, int ik2, int r2, int ik1,int r1,int sym ){
...
int main(int argc, char **argv){
   ...
   FILE *outfile_f_GEVP =NULL; mysprintf(namefile,NAMESIZE,"%s/out/f_PS_GEVP.txt",argv[3]);     outfile_f_GEVP=fopen(namefile,"w+");error(outfile_f==NULL,1,"main ", "Unable to open %s file",namefile);
...
////////////////////////////////////////////////////////////////////
 ...
 //  fclose(f);
   ...
   data=calloc_corr(confs, 6,  file_head.l0 );
...
//   for (ii=0;ii<si_2pt;ii++){
   ...
   for(imom1=0;imom1<file_head.nmoms;imom1++){
      ...
      // extract_twopt(out[i] ,data[i][0],si_2pt,ii,imom2,imom1,ik2,r2,ik1,r1 );
          ...
          //To compare what you are reading you should remember that in the reading function (read_twopt) there is the average r=+1 and -1 
      //if(  strcmp(name,"P5P5")==0 ) if(ik2==0) if (r2==0) if (ik1==0) if (r1==0) if (imom1==0) if (imom2==0) for(t=0;t<file_head.l0;t++){ printf("%d   %.15f   %.15f\n",t,data[i][4][t][0],data[i][4][t][1]);}
       }
       ...
       data_bin=binning(confs, 6, file_head.l0 ,data, bin);

       conf_jack=create_resampling(argv[4],Neff, 6, file_head.l0, data_bin);
...
mass_jack_fit_GEVP[ik2][ik1]=compute_effective_mass_GEVP(  argv, kinematic_2pt,  name, data,  confs, var ,plateaux_masses_GEVP,outfile_GEVP );
  */ 
      /* if (ik1==0 && ik2==0){
		      ...
		      // mass_jack_fit_GEVP[ik2][ik1]=compute_effective_mass_GEVP_out_max_twist(  argv, kinematic_2pt,  name, conf_jack,  Njack, var ,plateaux_masses_GEVP,outfile_GEVP );
		      // f_jack_fit[ik2][ik1]=compute_f_PS_ls_ss(  argv, kinematic_2pt,  name, conf_jack, mass_jack_fit_GEVP[ik2][ik1],  Njack ,plateaux_f,outfile_f_ls_ss );
               f_jack_fit[ik2][ik1]=compute_f_PS_ls_ss_out_max_twist(  argv, kinematic_2pt,  name, conf_jack, mass_jack_fit_GEVP[ik2][ik1],  Njack ,plateaux_f,plateaux_masses,outfile_f_ls_ss );
       ...
       if (ik1==0 &&  ik2<4){
	       ...
	       mass_jack_fit_GEVP[ik2][ik1]=compute_effective_mass_GEVP(  argv, kinematic_2pt,  name, conf_jack,  Njack, var ,plateaux_masses_GEVP,outfile_GEVP );
	       f_jack_fit[ik2][ik1]=compute_f_PS_ls_ss(  argv, kinematic_2pt,  name, conf_jack, mass_jack_fit_GEVP[ik2][ik1],  Njack ,plateaux_f,outfile_f_ls_ss );
	       f_jack_fit[ik2][ik1]=compute_Zf_PS_ll(  argv, kinematic_2pt,  name, conf_jack, mass_jack_fit[ik2][ik1],  Njack ,plateaux_masses,outfile_f );
       ...
       free_corr(Neff, 6, file_head.l0 ,data_bin);
       free_jack(Njack,6 , file_head.l0, conf_jack);
      ////////////////////GEVP f_PS
  /*   
...
////////                        RATIOS                                                    //////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////
   for(ik1=0;ik1<3;ik1++){     //for(ik1=0;ik1<=ik2;ik1++){
   for(r1=0;r1<1;r1++){  // it is not a loop, r1=0
   for(ik2=ik1;ik2<file_head.nk;ik2++){
   for(r2=0;r2<1;r2++){    // it is not a loop, r2=0
   for(imom2=0;imom2<file_head.nmoms;imom2++){
