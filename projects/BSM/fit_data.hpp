...
double rhs_NG_mpcac_MPS2_m0(int n, int Nvar, double* x, int Npar, double* P) {
    double eta = x[3], mu = x[5], m0 = x[6];
    double r;
    double eta_cr = x[Nvar - 2];
    double m0_cr = x[Nvar - 1];
    ...
    double m0_sub = m0 - m0_cr;
    if (n == 0) //m_pcac
        r = P[0] + P[2] * eta_sub + P[4] * mu + P[6] * m0_sub;
    else if (n == 1) //MPS^2
        r = P[1] + P[3] * eta_sub + P[5] * mu + P[7] * m0_sub;
    else { r = 0; exit(1); }
...
double rhs_NG_mpcac_MPS2_m0_eta2(int n, int Nvar, double* x, int Npar, double* P) {
    double eta = x[3], mu = x[5], m0 = x[6];
    double r;
    double eta_cr = x[Nvar - 2];
    double m0_cr = x[Nvar - 1];
...
struct fit_result fit_data(char** argv, vector<header_BSM> params, vector<data_BSM> gjack, double lhs_fun(int, int, int, vector<header_BSM>, vector<data_BSM>, struct fit_type), struct fit_type fit_info, const char* label, std::vector<int> myen) {
    ...
    for (int n = 0;n < N;n++) {
        for (int e = 0;e < en[n];e++) {
            ...
            printf(" %-10g%-10g%-10g%-10g  %g   %g\n", x[Njack - 1][e + count][3], x[Njack - 1][e + count][5],
                x[Njack - 1][e + count][6], x[Njack - 1][e + count][2],
                y[Njack - 1][e + count][0], y[Njack - 1][e + count][1]);
            free(tmpj);
        }
        count += en[n];
    }
